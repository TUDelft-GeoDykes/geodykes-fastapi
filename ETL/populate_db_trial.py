...
async def create_dykes(session: AsyncSession):
    # Read the Excel file
    excel_path = "/Users/chaoching-yu/Desktop/PostDoc/03_GeoDykeMonitor/Temp/geodykes-fastapi/tests/db/List.xlsx"
    sheet_name = "Dyke"
    df = pd.read_excel(excel_path, sheet_name=sheet_name)

    # Create Dyke instances
    dykes = [Dyke(id=row['Dyke ID'], name=row['Dyke Name'], description=row['Description']) for index, row in df.iterrows()]
...
async def create_crossections(session: AsyncSession, dykes):
    ...
    crossections = [Crossection(dyke_id=row['Dyke ID'], name=row['Cross Section Name'], description=row['Description'], topology=topology_data) for index, row in df.iterrows()]
...
async def create_units(session: AsyncSession):
    ...
    units = [UnitOfMeasure(id=row['Unit ID'],unit=row['Unit Name'], description=row['Description']) for index, row in df.iterrows()]
...
async def create_sensor_types(session: AsyncSession):
      # This will be created manually
    excel_path = "/Users/chaoching-yu/Desktop/PostDoc/03_GeoDykeMonitor/Temp/geodykes-fastapi/tests/db/List.xlsx"
    sheet_name = "Sensor Type"
    df = pd.read_excel(excel_path, sheet_name=sheet_name)

    sensor_types = [SensorType(id=row['Type ID'], name=row['Type Name'], details=row['Description'], multisensor=bool(1 % 2)) for index, row in df.iterrows()]
